...
/**
 * Analyzes comment using Chrome's Prompt API (client-side inference)
...
Comment to analyze: "${comment.replace(/"/g, '\\"')}"`;
    ...
    const responseText = parsePromptApiResponse(response, 'Client-side comment analysis');
    ...
    try {
      ...
      throw new Error(`Invalid response format from client-side AI: ${parseError.message}`);
    }
  } catch (error) {
...
/**
 * Analyzes comment using the best available AI: client-side first, server-side fallback
 * Uses a simpler check for comment analysis since it doesn't require model download
 * @param {string} comment - The comment text to analyze
 * @param {string} imageDescription - Optional description of the image being commented on
 * @returns {Object} Analysis result with isProblematic, reason, and suggestion
 */
...
export async function analyzeComment(comment, imageDescription = null) {
  return tryClientSideThenServerSide(
    analyzeClientComment,
    analyzeCommentWithGemini,
    'comment analysis',
    comment,
    imageDescription
  );
...
// These handle the comment form, status displays, and suggestion management
// =============================================================================

import { updateSubmitButton, escapeHtml, getElement, showSuccessNotification, showStatusNotification, hideElement, showElement, registerEventHandler } from '../../common/js/ui-helpers.js';
